...
public class WordReaderSpout extends BaseRichSpout {
	...
	public void nextTuple() {
		...
		try{
			...
			while((str = reader.readLine()) != null){
				System.out.println("The next line is ");
				System.out.println(str);
				StringTokenizer tokenizer = new StringTokenizer(str, "###");
				if (tokenizer.countTokens()==2) {
				String message = tokenizer.nextToken();
				String businessKeys = tokenizer.nextToken();
				/**
				 * By each line emit a new value with the line as a their
				 */
				this.collector.emit(new Values(message,businessKeys));
				}else {
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("payLoad","businessKeys"));
	}
}
